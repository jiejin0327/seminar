...
namespace KioskPhoneApp.NetCommom_Ting
{
    ...
    public class SrvClient
    {
        ...
        public SrvClient(string _Host)
        {
            ...
            {
                { MethodName.Register, MethodType.Post },
                { MethodName.GetToken, MethodType.Post },
                { MethodName.AiTraining, MethodType.Post },
            };
        }
        ...
        public async Task<ReqReturn> Register(string UserId, string UserPw, string UserName, string UserPhone, string UserEmail)
        {
            var Params = new UrlParam[]
            {
                new UrlParam("UserId",UserId),
                new UrlParam("UserPw",UserPw),
                new UrlParam("UserName",UserName),
                new UrlParam("UserPhone",UserPhone),
                new UrlParam("UserEmail",UserEmail),
            };

            var Ret = await Request(MethodName.Register, Params);
            return Ret;
        }
        ...
        public async Task<ReqReturn> UpToTraining(string FileName)
        {
            var Ret = await Request(MethodName.AiTraining, new UrlParam("AsUseType", "Training"), new UrlParam(FileName));
            return Ret;
        }
